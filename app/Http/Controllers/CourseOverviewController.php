...
class CourseOverviewController extends Controller
{
    ...
    public  function index(Request $request) {
        ...
        if ($request->ajax()){
            ...
            // If course state are provided, apply additional filtering
            if (!empty($course_state)) {
                // Find for the fields
                $query->whereIn('cricos.location_state', $course_state);
            }

            // If course search bar is not provided
            if (!empty($course_name)){
                // Find for the course name
                $query->whereRaw('LOWER(cricos.course_name) LIKE ?', ['%'.trim(strtolower($course_name)).'%']);
            }
